    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-data x-on:keydown.escape.window="$wire.cancelEdit()">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-lg font-semibold text-gray-800">Edit Order #{{ $editOrderNumber }}</h3>
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Order Number</label>
                    <input type="text" wire:model="editOrderNumber"
                    ...
                    @error('editOrderNumber') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Total Amount ($)</label>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">$</span>
                        <input type="number" step="0.01" min="0" wire:model="editTotalAmount"
                    ...
                    @error('editTotalAmount') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('editStatus') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>
